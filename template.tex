\documentclass[12pt]{article}

%%% Page setup %%%
\usepackage[top=1in, bottom=1in, left=1.2in, right=1in]{geometry}
\pagestyle{plain}
\linespread{1.5}

%%% Packages %%%
\usepackage[english]{babel}
\usepackage[utf8]{inputenc}
\usepackage{amsmath}
\usepackage[toc,page]{appendix}
\usepackage{fancyhdr}
\usepackage{gensymb}
\usepackage{geometry}
\usepackage{graphicx}
\usepackage{listings}
\usepackage{parskip}
\usepackage{tabularx}
\usepackage{tikz}
\usepackage{wallpaper}
\usepackage{wrapfig}

\usepackage[hypertexnames=false]{hyperref}
\usepackage[numbered, depth=3]{bookmark}

\usepackage[backend=biber,style=apa]{biblatex}
\addbibresource{template.bib}

\usetikzlibrary{matrix}


%%% Variables %%%
\newcommand{\faculty}{Faculty of Science and Engineering}
\newcommand{\course}{Course Name}
\newcommand{\documenttitle}{Document Title}
\newcommand{\authorname}{Firstname Lastname}
\newcommand{\authoremail}{firstname.lastname@abo.fi}
\newcommand{\documentdate}{\today}

%%% Document %%%
\begin{document}
	\begin{titlepage}
		\begin{center}
			\textsc{\AA bo Akademi University} \\
			\textsc{\faculty}
		\end{center}
		\begin{figure}[h]
			\vspace{10mm}

			\centering\includegraphics[width=0.4\textwidth]{logo.eps}

			\vspace{20mm}
		\end{figure}
		\begin{center}
			\fontsize{10mm}{7mm}\selectfont
			\textup{\documenttitle} \\
			\textnormal{ \Large{\course}} \\

			\vspace{25mm}

			\large{\textbf{\authorname}} \\
			\large{\href{mailto:\authoremail}{\authoremail}} \\

			\vspace{35mm}

			\documentdate
		\end{center}
	\end{titlepage}

	\clearpage

	%%% Table of contents %%%

	\tableofcontents

	\cleardoublepage

	%%% Document body %%%

	\section{First section}

	\subsection{First subsection}

	Logos and illustrations provided by \citeauthor{AboAkademi2023}.

	\clearpage

	%%% Bibliography %%%

	\printbibliography[heading=bibintoc]

\end{document}
